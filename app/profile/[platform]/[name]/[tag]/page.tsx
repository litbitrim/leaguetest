...
function ChampionStatsTab({allMatches,puuid}:{allMatches:any[],puuid:string}) {
  ...
  return <div>
    <div style={{display:'flex',gap:'6px',marginBottom:'16px',alignItems:'center',flexWrap:'wrap'}}>
      ...
      <span style={{fontSize:'12px',color:muted,marginLeft:'8px'}}>{filtered.length} games</span>
    </div>
    <div style={{background:bg2,border,borderRadius:'12px',overflow:'auto'}}>
      ...
      {list.length===0&&<div style={{padding:'40px',textAlign:'center',color:muted}}>No games found</div>}
    </div>
  </div>
}
...
export default function ProfilePage() {
  ...
  const titles = d ? deriveTitles(d.allMatches||d.matches||[], d.account.puuid) : []
  const allMatches = d?.allMatches || d?.matches || []
  ...
  // Champion stats from ALL matches
  ...
  if(loading||!d) return <main style={{background:bg,minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',fontFamily:'system-ui,sans-serif',gap:'12px'}}>
    ...
    <div style={{color:'#555d78',fontSize:'12px'}}>Fetching up to 2500 ranked games, this may take a moment</div>
  </main>
  ...
  return <main style={{background:bg,minHeight:'100vh',color:'#e8eaf2',fontFamily:'system-ui,sans-serif'}}>
    <div style={{background:bg2,borderBottom:border,padding:'0 24px',display:'flex',alignItems:'center',gap:'12px',height:'52px'}}>
      ...
      {d.totalFetched&&<span style={{fontSize:'11px',color:muted,marginLeft:'auto'}}>{d.totalFetched} ranked games loaded</span>}
    </div>
    ...
    <div style={{maxWidth:'1200px',margin:'0 auto',padding:'20px'}}>
      ...
      <div style={{...card,display:'flex',alignItems:'center',gap:'20px',marginBottom:'12px',flexWrap:'wrap'}}>
        ...
        <div style={{display:'flex',gap:'8px'}}>
          {d.liveGame&&<button style={{background:'rgba(247,90,90,0.12)',border:'1px solid rgba(247,90,90,0.3)',borderRadius:'10px',padding:'8px 16px',color:loss,fontWeight:600,cursor:'pointer',fontSize:'13px'}}>🔴 Live</button>}
      ...
      {activeTab==='champions' ? <ChampionStatsTab allMatches={allMatches} puuid={d.account.puuid}/> : (
        <div style={{display:'grid',gridTemplateColumns:'280px 1fr',gap:'14px'}}>
            ...
            <div style={card}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'12px'}}>
                <div style={{fontSize:'10px',fontWeight:700,color:muted,textTransform:'uppercase',letterSpacing:'1px'}}>
                  Most Played <span style={{color:'#333a50'}}>({allMatches.length} games)</span>
                </div>
          ...
          <div>
            <div style={{fontSize:'10px',fontWeight:700,color:muted,textTransform:'uppercase',letterSpacing:'1px',marginBottom:'10px'}}>
              Match History · {d.matches?.length} recent · {allMatches.length} total loaded
            </div>
            {d.matches?.map((m:any)=>{
              const me=m.info?.participants?.find((p:any)=>p.puuid===d.account.puuid)
              if(!me) return null
